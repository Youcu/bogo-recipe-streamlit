...
# 이전 카테고리 상태 유지
if 'previous_filtered_category' not in st.session_state:
    st.session_state.previous_filtered_category = "전체"
...
selected_category_name = st.selectbox("Recipe Category", category_names, index=category_names.index(st.session_state.previous_filtered_category))

# 선택한 카테고리와 이전 카테고리를 비교해 다르면 페이지 리셋
if selected_category_name != st.session_state.previous_filtered_category:
    st.session_state.filtered_page = 1
    st.session_state.previous_filtered_category = selected_category_name
